...
class Chanel
{
		...
		unsigned int					_limit; // limite d utilisateurs sur le chanel, 0 pour pas de limite.
	...
	public :
										Chanel( void ) : _mod (0), _priv (0), _secret(0), _limit (0), _extMsg(0) {}
										Chanel( std::string str ) : _name( str ), _mod (0), _priv(0), _secret(0), _limit(0), _extMsg(0) {}
										~Chanel( void ) {}
		...
		void							eraseCli( std::string user )
		{
			...
			if ( cliIt == _cliCrnt.end() )
				throw(IRCErr("Client isnt in the chanel: " + _name));
			_cliCrnt.erase(cliIt);
		}

		void							setVo( std::string tmp ) { _cliVo.push_back( tmp ); }
		std::list<std::string>			*getVo( void ) { return &_cliVo; }
		strIt							isVo( std::string user ) {
			strIt voIt;
			for ( strIt voIt = _cliVo.begin(); voIt != _cliVo.end() && user != *voIt; ++voIt );
			return voIt;
		}
		void                            unsetVo( std::string user )
        {
			strIt voIt = isVo( user );
            if ( voIt != _cliVo.end() )
				_cliVo.erase( voIt );
        }
