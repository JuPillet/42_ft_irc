...
class Channel
{
    ...
    public :
		...
		bool        isOps (Client *tmp) {
		    for ( ClientIt tmpIt = _chanOps.begin(); tmpIt != _chanOps.end(); ++tmpIt ) {
		        if ( *tmpIt == tmp )
		            return (1);
		    }
		    return (0);
		}
		void        setOps( Client *tmp ) { _chanOps.push_back(tmp); }

		void		setCli( Client *tmp ) { _cli.push_back(tmp); }
		bool        isCli ( Client *tmp ) {
		    for ( ClientIt tmpIt = _cli.begin(); tmpIt != _cli.end(); ++tmpIt ) {
		        if ( *tmpIt == tmp )
		            return (1);
		    }
		    return (0);
		...
		bool        isVo (Client *tmp) {
		    for ( ClientIt tmpIt = _vo.begin(); tmpIt != _vo.end(); ++tmpIt ){
		        if ( *tmpIt == tmp )
		            return (1);
		    }
		    return (0);
